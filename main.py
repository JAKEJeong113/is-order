...
cu_price_crawl.init_cu_price_overrides_table()
cu_price_crawl.init_coupang_price_exclusions_table()
cu_price_crawl.init_coupang_wholesale_costs_table()
...
# 수렴한다. 이 API는 공식 한도가 분당 50회로 엄격해서(초과 시 경고, 경고
# 3회 누적되면 계정 자체가 제한됨) 자주 돌리면 안 되지만, 위와 같은
# 이유로 매일 돌려도 안전하다. 실제 한도 초과 방지는 이 스케줄이 아니라
# product_ranking.search_coupang_product() 내부의 분당 호출수 제한이
# 담당한다(모든 호출 경로 공통) - 여기 배치 크기/주기는 그냥 합리적인
# 페이싱일 뿐, 안전장치의 최후 방어선이 아니다.
#
...
# 226개(음료 149 + 과자 77) 전체를 한 번에 조회하면 분당 호출 한도(공식
# 분당 50회, 안전마진 적용 분당 35회)를 넘으므로 15개씩 30분 간격으로
# 나눠 돈다 - 짧은 시간에 15건이 몰려도 분당 한도 이내라 안전하다. 전체
# 카탈로그 한 바퀴 도는 데 약 8시간이라 하루 2~3회 자연스럽게 갱신된다.
#
