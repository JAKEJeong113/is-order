...
class ScanActivity : AppCompatActivity() {
    ...
    private fun isPlausibleBarcode(code: String): Boolean {
        ...
        // 있다 - 아래 표준 체크섬은 EAN-8엔 맞지만 UPC-E엔 안 맞는다(UPC-E는
        // 12자리 UPC-A로 먼저 복원한 뒤 그 값으로 체크섬을 계산해야 함).
        // 8자리는 둘 다 시도해서 하나라도 맞으면 통과시킨다.
    ...
    private fun isValidUpcE(code: String): Boolean {
        ...
        val total = upcA11.foldIndexed(0) { index, acc, digit -> acc + digit * (if (index % 2 == 0) 3 else 1) }
