...
def _extract_unit_qty(text: str) -> int | None:
    ...
    "(1박스 46개입)"(박 뒤에 "스"가 붙는 표기 - 실측: 과자생각 상품 176개가 이
    표기라 "박"만 잡던 예전 패턴에 안 걸려서 카탈로그 자동등록 때 통째로
    누락됐었음), 현동몰의 "(36입)" 표기도 순서대로 시도한다(우선순위: 더
    구체적인 패턴 먼저)."""
    if not text:
        return None
    ...
    match = re.search(r"1(?:타|묶음|박스?)\s*(\d+)\s*개입", text)
    if match:
        return int(match.group(1))
