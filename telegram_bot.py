...
# 엑셀 카탈로그에 아직 없는 신상품을 DB에 직접 등록하는 명령 - 가맹점이 마음대로
# 카탈로그를 오염시키면 안 되므로 대표님(ADMIN_CHAT_ID) 전용이다.
BARCODE_ADD_TRIGGER_WORDS = {"바코드추가", "바코드등록", "바코드수정"}
# 아래 둘은 웹 페이지로 유도하는 게 아니라, 등록된 상품명+구매링크를 텔레그램
# 안에서 바로 보여준다(biz_tools.py/consumables.py가 각각 관리하는 데이터 그대로).
HONEY_ITEM_TRIGGER_WORDS = {"꿀템추천", "영업꿀템"}
CONSUMABLE_TRIGGER_WORDS = {"소모품", "소모품추천"}
...
def _handle_barcode_add_reply(chat_id: str, state: dict, text: str) -> None:
    stripped = text.strip()
    if stripped.lower() in CANCEL_WORDS:
        telegram_store.set_disambig_state(chat_id, None)
        send_message(chat_id, "바코드 등록을 취소했습니다.")
        return

    step = state["step"]

    if step == "barcode":
        state["barcode"] = stripped
        state["step"] = "name"

        # 이미 등록된 바코드면(엑셀 원본이든 이전에 등록한 값이든) 현재 값을
        # 먼저 보여준다 - "추가"와 "수정"이 같은 흐름(upsert)이라, 덮어쓰기
        # 전에 지금 뭐가 등록되어 있는지 확인할 수 있게 하기 위함.
        existing = next((e for e in product_ranking.search_catalog(stripped, limit=5) if e["barcode"] == stripped), None)
        state["was_existing"] = existing is not None
        telegram_store.set_disambig_state(chat_id, state)

        if existing:
            price_text = f"{existing['recommended_price']:,}원" if existing.get("recommended_price") else "미등록"
            send_message(
                chat_id,
                f"이미 등록된 바코드예요.\n현재 제품명: {existing['menu_name']}\n현재 추천판매가: {price_text}\n\n"
                "새 제품명을 입력해주세요(그대로 두려면 같은 이름을 다시 입력해주세요).\n(취소하려면 '취소')",
            )
        else:
            send_message(chat_id, "제품명을 입력해주세요.\n(취소하려면 '취소')")
        return

    if step == "name":
        state["menu_name"] = stripped
        state["step"] = "price"
        telegram_store.set_disambig_state(chat_id, state)
        send_message(chat_id, "추천판매가를 입력해주세요. (숫자만)\n(취소하려면 '취소')")
        return

    # step == "price"
    if not stripped.isdigit():
        send_message(chat_id, "숫자만 입력해주세요. 추천판매가를 다시 입력해주세요.")
        return

    price = int(stripped)
    telegram_store.set_disambig_state(chat_id, None)
    product_ranking.add_custom_catalog_item(state["barcode"], state["menu_name"], price)
    verb = "수정되었습니다" if state.get("was_existing") else "등록되었습니다"
    send_message(
        chat_id,
        f"{verb}.\n제품명: {state['menu_name']}\n바코드: {state['barcode']}\n추천판매가: {price:,}원",
    )
...
def _candidate_caption(index: int, group: dict) -> str:
...
def handle_update(update: dict) -> None:
    ...
    if text.strip() in BARCODE_ADD_TRIGGER_WORDS:
        if chat_id != ADMIN_CHAT_ID:
            send_message(chat_id, "이 기능은 관리자만 사용할 수 있어요.")
            return
        state = {"mode": "barcode_add", "current": True, "step": "barcode"}
        telegram_store.set_disambig_state(chat_id, state)
        send_message(chat_id, "추가할 바코드를 입력해주세요.\n(취소하려면 '취소')")
        return

    if text.strip() in HONEY_ITEM_TRIGGER_WORDS:
        send_message(chat_id, _format_link_list("영업 꿀템 추천", biz_tools.list_tools()))
        return

    if text.strip() in CONSUMABLE_TRIGGER_WORDS:
        send_message(chat_id, _format_link_list("소모품", consumables.list_items()))
        return
